    ...
    handleVoteSubmit(event, pollIndex) {
        ...
        if(!this.props.isAuthenticated) {
            ...
            notification.info({
                ...
                description: "Please login to support.",          
            });
            return;
        }
        ...
        .then(response => {
            ...
            if(error.status === 401) {
                this.props.handleLogout('/login', 'error', 'You have been logged out. Please login to support');    
            } else {
                notification.error({
                    message: 'Petition Signing App',
    ...
    render() {
        ...
        return (
            <div className="polls-container">
                ...
                Rate this App: 
                <input type="radio" name="rate" class="rate"/>🤩  
                <input type="radio" name="rate" class="rate"/>😊
                <input type="radio" name="rate" class="rate"/>😐
                <input type="radio" name="rate" class="rate"/>😣
                <input type="radio" name="rate" class="rate"/>😥
